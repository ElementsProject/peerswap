...
func TestOnPremiumUpdateForcesPoll(t *testing.T) {
	syncer, deps := newTestPeerSync(t)

	peerID, err := NewPeerID("peer-update")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	peer := NewPeer(peerID, "addr-update")
	...
	syncer.OnPremiumUpdate()

	deadline := time.Now().Add(500 * time.Millisecond)
	for time.Now().Before(deadline) {
		if deps.lightning.SentCount() > 0 {
			break
		}
	...
	if deps.lightning.SentCount() == 0 {
		t.Fatalf("expected forced poll after premium update")
	}
}
