...
func (cl *ClightningClient) CreateOpeningTransaction(swapParams *swap.OpeningParams) (unpreparedTxHex, txId string, fee uint64, vout uint32, err error) {
	...
	if unpreparedTxId, ok = cl.hexToIdMap[unpreparedTxHex]; !ok {
		return "", "", 0, 0, errors.New("tx was not prepared not found in map")
	}
	...
	return sendRes.SignedTx, sendRes.TxId, 0, 0, nil
}
