...
#         elementsd.rpc.sendtoaddress(addr, 0.1, "", "", False, False, 1, "UNSET")
    addr = nodes[1].rpc.call("peerswap-liquid-getaddress")["liquid_address"]
    elementsd.rpc.sendtoaddress(addr, 0.1, "", "", False, False, 1, "UNSET")
    ...
    balances_invoice_payed = [x.rpc.call("peerswap-liquid-getbalance")["liquid_balance_sat"] for x in nodes]
    assert balances_invoice_payed[0] == 0
    # FIXME correct fee
...
def test_swap_out_cooperative_close(elementsd: ElementsD, node_factory: NodeFactory):
    FUNDAMOUNT = 10 ** 7
    ...
    assert chfunds == FUNDAMOUNT - 6 * 10 ** 6 - FEE


    elementsd.rpc.generatetoaddress(2, BURN_ADDR)
    try:
        nodes[1].daemon.wait_for_log("Event_ActionSucceeded on State_SwapOutReceiver_ClaimSwapCoop")
    except Exception as e:
        print("FAILED")
        pytest.fail()
